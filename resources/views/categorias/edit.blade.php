    ...
    <div class="max-w-lg mx-auto bg-white rounded shadow p-6">
        ...
        <form action="{{ route('categorias.update', $categoria) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nome" value="{{ $categoria->nome }}" required class="w-full border rounded px-3 py-2">
            </div>
            <div class="flex justify-end gap-2">
                <button type="submit" class="bg-yellow-600 text-white px-4 py-2 rounded hover:bg-yellow-700">Atualizar</button>
                <a href="{{ route('categorias.index') }}" class="bg-gray-200 text-gray-800 px-4 py-2 rounded hover:bg-gray-300">Voltar</a>
            </div>
        </form>
    </div>
@endsection 